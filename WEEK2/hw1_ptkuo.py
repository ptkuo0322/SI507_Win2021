...
% I worked with the following classmates: None, I work along to finish this HW.
...
def make_move(player, board):
    ...
    while True:
        try:
            move = int(input("Where would you like to move? Enter a number between 1-9 "))
            move -= 1
...
def check_win_horizontal(board):
    ...
    -------
    int
        the player ID of the winner. 0 means no winner (no winning 
        condition in horizontal direction (row) is satisfied)
    '''
    if (board[0] != 0 and 
        board[0] == board[1] and 
        board[0] == board[2]):
...
def check_win_vertical(board):
    ...
    int
        the player ID of the winner. 0 means no winner (no winning 
        condition in vertical direction (column) is satisfied)
    '''
...
def check_win_diagonal(board):
    ...
    int
        The player ID of the winner. 0 means no winner (no winning 
        condition in diagonal direction  is satisfied)
    '''
    # TODO: implement function
    if (board[0] != 0 and 
        board[0] == board[4] and 
